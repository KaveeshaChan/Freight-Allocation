...
const PDFGenerator = ({ order, freightQuotes }) => {
  ...
  const printPDF = () => {
    ...
    img.onload = () => {
      ...
      doc.setFontSize(9);
      doc.text(`- Order Type: Import - AirFreight`, col1, yPos + lineHeight);
      ...
      doc.text(`- Cargo CBM: ${(order.cargoType || 0).toLocaleString()}`, col2, yPos + lineHeight * 5);
      doc.text(`- L*W*H of pallet: ${order.LWHWithThePallet|| 'N/A'}`, col2, yPos + lineHeight * 6);
yPos += 45;
      ...
      if (freightQuotes?.length > 0) {
        ...
        const headers = [
          ...
          { header: 'Air Freight Cost', dataKey: 'airFreightcost' },
