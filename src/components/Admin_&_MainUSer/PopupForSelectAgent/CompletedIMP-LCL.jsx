...
const QuoteDetailsPopup = ({ quote, order, onClose, onSelectAgent }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch('http://localhost:5056/api/update/update-preQuotes/import-lcl', {
        method: 'POST',
        headers: {
          ...
          <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-xl w-full">
              ...
              <div className='flex justify-between flex-wrap gap-4 p-4'>
                ...
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">DO Fee ($)</label>
                  <input name='DOFee' type="text" defaultValue={order.DOFee } value={formData.DOFee || 'N/A'} onChange={handleChange} className="w-full border rounded px-3 py-2" />
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Free Time</label>
                  <input name='freeTime' type="text" value={formData.freeTime || ''} onChange={handleChange} className="w-full border rounded px-3 py-2" />
                </div>
                <div className="mb-4">
