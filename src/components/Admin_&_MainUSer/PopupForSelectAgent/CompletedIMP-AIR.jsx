...
const QuoteDetailsPopup = ({ quote, order, onClose, onSelectAgent }) => {
  ...
  useEffect(() => {
    if (showEditDialog && order) {
      setFormData({
        airFreightCost: order.airFreightCost,
        AWB: order.AWB,
  ...
  const handleSave = async () => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch('http://localhost:5056/api/update/update-preQuotes/import-air', {
        method: 'POST',
        headers: {
          ...
          <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-xl w-full">
              ...
              <div className='flex justify-between flex-wrap gap-4 p-4'>
                <div className="mb-4 ">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Air Freight Cost</label>
                  <input name="airFreightCost" type="text" defaultValue={order.airFreightCost} value={formData.airFreightCost || ''} onChange={handleChange} className="w-full border rounded px-3 py-2" />
                </div>
                <div className="mb-4">
