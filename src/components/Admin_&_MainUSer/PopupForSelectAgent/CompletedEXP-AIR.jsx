...
const QuoteDetailsPopup = ({ quote, order, onClose, onSelectAgent }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setIsLoading(true)
      const response = await fetch('http://localhost:5056/api/update/update-preQuotes/export-air', {
        method: 'POST',
        headers: {
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-xl w-full">
              ...
              <div className='flex justify-between flex-wrap gap-4 p-4'>
                ...
                <div className="mb-4">
                  ...
                  <input name='AWB' type="text" defaultValue={order.AWB} value={formData.AWB || ''} onChange={handleChange} className="w-full border rounded px-3 py-2" />
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">HAWB($)</label>
                  <input name='HAWB' type="text" defaultValue={order.HAWB} value={formData.HAWB || ''} onChange={handleChange} className="w-full border rounded px-3 py-2" />
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Air Line</label>
                  <input name='airLine' type="text" defaultValue={order.airLine} value={formData.airLine || ''} onChange={handleChange} className="w-full border rounded px-3 py-2" />
                </div>
                <div className="mb-4">
              ...
              <div className="flex justify-end gap-2 mt-6">
                <button onClick={handleDialogClose} className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
                <button
                  onClick={handleSave}
